...
| II | Parte II — Programação (Seções 8–11) | ✅ feito | **Modo programa completo**: P/R, gravação com keycodes (multi-tecla STO+1=`44 40 1`), SST/BST, g GTO . nnn, CLEAR PRGM, R/S roda, PSE; **branching/looping (Seção 9): x≤y/x=0 (DO-if-TRUE) + GTO desvio/loop**; **edição (Seção 10): substituição de linha**; **múltiplos programas (Seção 11): separador GTO 000 + rodar via g GTO nnn em run**. Ex.: desconto+frete (625→473,75), fatura+acumuladores (→950,61), imposto 20%/25% condicional (15k→3.000/20k→4.000/25k→6.250), edição (625→505), 2 programas (×/+). |
