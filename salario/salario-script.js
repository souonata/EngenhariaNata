...
function calcularIT(v) {
    const ral = v.bruto * 12;                          // bruto é mensal; RAL anual
    const ralMensile = v.bruto;
    const inps = calcularINPS(ralMensile);
    const inpsAnnuo = inps * 12;
    ...
    // Mensile (distribuito su 12 mesi)
    const irpefMensile = irpefNetta / 12;
    ...
    const outros = v.outros;
    const totalDescontos = inps + irpefMensile + addRegionalMensile + addComunaleMensile + outros - trattamentoMensile;
    const liquido = ralMensile - totalDescontos - (v.plano /* previdenza deducted mensalmente */);

    // Tredicesima
    let tredicesimaNetta = 0, tredicesimaBruta = 0;
    if (v.tredicesima === 'sim') {
        tredicesimaBruta = ralMensile;
        const trInps = calcularINPS(tredicesimaBruta);
        const trImp = tredicesimaBruta - trInps;
        // 13ª tassata come reddito senza detrazioni lavoro (più pesante)
        const trIrpef = calcularIRPEFMarginal(imponibileAnnuo, trImp);
        tredicesimaNetta = tredicesimaBruta - trInps - trIrpef;
    }
    ...
    if (v.quattordicesima === 'sim') {
        const q = ralMensile;
        const qInps = calcularINPS(q);
        const qImp = q - qInps;
        const qIrpef = calcularIRPEFMarginal(imponibileAnnuo, qImp);
        quattordicesimaNetta = q - qInps - qIrpef;
    }

    // TFR
    const tfrMensile = ralMensile * TFR_IT_ALIQ_NETA;
    const anni = v.meses / 12;
    let tfrAccumulato;
    ...
    // Costo azienda
    const custoEmpresa = ralMensile * COSTO_AZIENDA_IT;
    ...
    return {
        ...
        feriasBruto: 0, feriasLiquido: quattordicesimaNetta,
...
class SalarioApp extends App {
    ...
    configurarIconesInfo() {
        ['Bruto','Dependentes','Plano','Outros','Meses'].forEach(sufixo => {
            const icon = document.getElementById('infoIcon' + sufixo);
            const desc = document.getElementById('descricao' + sufixo);
            if (!icon || !desc) return;
            const toggle = () => {
    ...
    gerarExplicacao(r) {
        ...
        return {
            linhas,
            destaque: isIt
                ? `Netto mensile: ${this.fMoeda(liq)} (${pct.toFixed(1)}% della RAL)`
